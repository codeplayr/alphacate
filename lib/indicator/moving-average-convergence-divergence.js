...
MACD.prototype = {
	...
	_compute(){
		...
		let {slowPeriods, fastPeriods, signalPeriods, includeOffset} = this._options;
		...
		if( includeOffset ){
			for(let i=0; i<slowPeriods - 1; i++){
				if( i>signalPeriods -1 ) results_signal.unshift(0);
		...
		return {
			...
			fast_ema:  (includeOffset) ? results_fast_orig : results_fast,
			fast_ema_orig:  results_fast_orig,
